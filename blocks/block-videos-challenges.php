...
$bouton_filtre_tous_les_challenges = get_field('bouton_filtre_tous_les_challenges', 'option') ?? '';
$bouton_filtre_toutes_les_categories = get_field('bouton_filtre_toutes_les_categories', 'option') ?? '';
$display_3 = get_field('display_3', 'option') ?? '';
$display_6 = get_field('display_6', 'option') ?? '';
$display_9 = get_field('display_9', 'option') ?? '';
...
<div style="<?= esc_attr($style) ?>" class="block-videos block-videos-challenges wp-block-acf <?= esc_html($class) ?>">
    ...
    <div id="filters">
        <div class="first-group">
            <div class="filter-group" data-filter-group="challenge">
                <button data-filter="all" class="active"><?= esc_html($bouton_filtre_tous_les_challenges ?? '') ?></button>
                <button data-filter="challenge-1">Challenge 1</button>
                <button data-filter="challenge-2">Challenge 2</button>
                <button data-filter="challenge-3">Challenge 3</button>
            </div>
            <?php
            $terms = get_terms([
            ...
            if (!empty($terms) && !is_wp_error($terms)) : ?>
                <div class="filter-group" data-filter-group="topic">
                    <button data-filter="all" class="active"><?= esc_html($bouton_filtre_toutes_les_categories) ?? '' ?></button>
